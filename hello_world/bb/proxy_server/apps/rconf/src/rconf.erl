...
read_config(proxy_server) -> 
	case read_config_file() of
		{ok, Config} -> 
			{_, {proxy_server, ProxyServerConfig}, _ } = lists:keytake(proxy_server, 1, Config),
			{_, {host, Host}, _} = lists:keytake(host, 1, ProxyServerConfig),
			{_, {port, Port}, _} = lists:keytake(port, 1, ProxyServerConfig),
			{_, {id, Id}, _} = lists:keytake(id, 1, ProxyServerConfig),
			{Host, to_integer(Port), Id};
		_ -> 
			ok
	end;
