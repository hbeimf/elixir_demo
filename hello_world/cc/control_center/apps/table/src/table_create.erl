...
init_slave() ->
	...
	case mnesia:change_config(extra_db_nodes, [MasterNode]) of
        {ok, [MasterNode]} ->
        	?LOG1({"init_slave"}),
            Res1 = mnesia:add_table_copy(gateway_list, node(), ram_copies),
            Res2 = mnesia:add_table_copy(game_server_list, node(), ram_copies),
            Res3 = mnesia:add_table_copy(client_list, node(), ram_copies),
            Res4 = mnesia:add_table_copy(client_counter, node(), ram_copies),
            Res5 = mnesia:add_table_copy(forbidden_ip, node(), ram_copies),


            ?LOG1({Res1, Res2, Res3, Res4, Res5}),
