...
class ReleationbindController extends AbstractController {
	...
	public function listAction() {
		...
		$table_user = Table_Logic_Schooltype::selectRaw($select);
		if (trim($params['name']) != '') {
	...
	public function addAction() {
		if ($this->request->isPost()) {
			...
			if ($id == '') {
				...
				DB::table('t_school_type')->insert([$data]);
		...
		if (! is_null($this->request->getParam('id'))) {
			...
			$role = DB::table('t_school_type')->where('id', $id)->first();

			$this->smarty->assign('role', $role);
		}

		$this->smarty->display('schooltype/add.tpl');
	}
