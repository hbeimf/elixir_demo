...
class TutorialController extends AbstractController {
	...
	public function listAction() {
		...
		$table_user = Table_Logic_Tutorial::selectRaw($select);
		if (trim($params['name']) != '') {
		...
		$data = [
			...
			'params' => $params,
			'has_add_right' => $this->has_right('tutorial', 'add'),
			'has_enable_right' => $this->has_right('tutorial', 'enable'),
			'has_unenable_right' => $this->has_right('tutorial', 'unenable'),
		];

		$this->smarty->display('tutorial/list.tpl', $data);
	}
	...
	public function addAction() {
		if ($this->request->isPost()) {
			...
			if ($data['name'] == '') {
				return $this->ajax_error('教材名称不能为空');
			}
			...
			if ($id == '') {
				...
				DB::table('t_tutorial')->insert([$data]);
				return $this->ajax_success('添加成功！');
			} else {
				$data['updated_at'] = time();
				Table_Logic_Tutorial::where('id', $id)->update($data);
		...
		if (! is_null($this->request->getParam('id'))) {
			...
			$role = DB::table('t_tutorial')->where('id', $id)->first();

			$this->smarty->assign('role', $role);
		}

		$this->smarty->display('tutorial/add.tpl');
	}
