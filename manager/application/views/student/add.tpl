<form name="ff" id="ff" class="form-horizontal ajax_form" action="/student/add" method='post' enctype="multipart/form-data">
    <input type="hidden" name="id" value="{{if isset($role['id'])}}{{$role['id']}}{{/if}}" />
    <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-hidden="true"></button>
         <h4 class="modal-title">{{if isset($role['id'])}}编辑{{else}}新增{{/if}}学生</h4>
    </div>
...
<div class="control-group">
    ...
    <div class="controls">
        <select name="class_id" class="span6 m-wrap select2" placeholder="请选择班级...">
            ...
            {{foreach from=$classes item=m}}
                <option {{if isset($role['class_id']) && $role['class_id'] == $m['id']}}selected{{/if}} value="{{$m['id']}}">{{$m['name']}}</option>
            {{/foreach}}
        </select>
    </div>
</div>
