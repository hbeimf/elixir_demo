        ...
        <div class="portlet-body">
            <div class="clearfix">
                ...
                <div class="btn-group pull-right">
                    <a data-toggle="modal" data-target="#mod_1200" href="/ppt/add/?curriculum_id={{$params['curriculum_id']}}" class="btn green">
            ...
            <table class="table table-striped table-bordered table-hover" id="sample_1">
                ...
                <tbody>
                    ...
                    <tr class="odd gradeX">
                        ...
                        <td>{{if $r['is_enabled']==1}}启用{{else}}
                            <font color=red>禁用</font>{{/if}}</td>
                        <td>{{$r['created_at']}}</td>
                        <td>{{$r['updated_at']}}</td>
                        <td>
                            {{if $has_add_right['flg']}}
                            <a data-toggle="modal" data-target="#mod_1200" href="/ppt/add/id/{{$r['id']}}/?curriculum_id={{$r['curriculum_id']}}" class="btn grey">
                                    <i class="fa fa-pencil"></i>编辑
                                </a> {{/if}} {{if $r['is_enabled'] == 1}} {{if $has_unenable_right['flg']}}
                            <a data-link="/ppt/unenable/id/{{$r['id']}}/" class="btn red ajax-delete" data-msg="确认要禁用吗？">
                                    <i class="fa fa-pencil"></i>禁用
                                </a> {{/if}} {{else}} {{if $has_enable_right['flg']}}
                            <a data-link="/ppt/enable/id/{{$r['id']}}/" class="btn green ajax-delete" data-msg="确认要启用吗？">
