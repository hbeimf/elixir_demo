<form name="ff" id="ff" class="form-horizontal ajax_form" action="/music/add/?curriculum_id={{$curriculum_id}}" method='post' enctype="multipart/form-data">
    <input type="hidden" name="id" value="{{if isset($role['id'])}}{{$role['id']}}{{/if}}" />
    <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-hidden="true"></button>
         <h4 class="modal-title">{{if isset($role['id'])}}编辑{{else}}新增{{/if}}乐谱</h4>
    </div>
...
<div class="control-group">
    ...
    <div class="controls">
        <select name="tutorial_id" class="span6 m-wrap select2" placeholder="请选择学校...">
            ...
            {{foreach from=$tutorial item=m}}
                <option {{if isset($role['tutorial_id']) && $role['tutorial_id'] == $m['id']}}selected{{/if}} value="{{$m['id']}}">{{$m['name']}}</option>
            {{/foreach}}
        </select>
    </div>
</div>
