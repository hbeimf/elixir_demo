<form name="ff" id="ff" class="form-horizontal ajax_form" action="/pic/add/?curriculum_id={{$curriculum_id}}" method='post' enctype="multipart/form-data">
    <input type="hidden" name="id" value="{{if isset($role['id'])}}{{$role['id']}}{{/if}}" />
    <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-hidden="true"></button>
         <h4 class="modal-title">{{if isset($role['id'])}}编辑{{else}}新增{{/if}}图片</h4>
    </div>
    ...
    <div class="controls">
        <div class="span6 fileupload fileupload-new" data-provides="fileupload">
            <div class="fileupload-new thumbnail" style="width: 200px; height: 150px;">
                <img src="{{if isset($pic['dir'])}}{{$pic['dir']}}{{else}}/image/AAAAAA&amp;text=no+image{{/if}}" alt="" />
            </div>
