<form name="ff" id="ff" class="form-horizontal ajax_form" action="/system/addRole" method='post'>
    ...
    <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-hidden="true"></button>
         <h4 class="modal-title">{{if isset($role['id'])}}编辑{{else}}新增{{/if}}角色</h4>
    </div>
...
<div class="control-group">
    ...
    <div class="controls">
        ...
        <input value="{{if isset($role['role_name'])}}{{$role['role_name']}}{{/if}}"
            name="role_name" type="text" placeholder="small" class="m-wrap span6" {{if isset($role['id'])}}readonly{{/if}}  />
                ...
                {{foreach from=$system_menu item=m}}
                    <optgroup label="{{$m['menu_name']}}">
                        {{foreach from=$m['child'] item=mm}}
                            ...
                            value="{{$mm['id']}}">{{$mm['menu_name']}}-{{$mm['controller']}}/{{$mm['action']}}</option>
                        {{/foreach}}
                    </optgroup>
                {{/foreach}}
            </select>
...
<div class="control-group">
    ...
    <div class="controls">
        <label class="radio">
        <input {{if isset($role['status']) && $role['status']=='1'}}checked{{/if}}{{if !isset($role['status']) }}checked{{/if}}
